...
def create_word_document(tables, output_path):
    ...
    for sheet_name, table_data in tables:
        ...
        hdr_cells[0].text = 'OS Ticket Numbers'
        hdr_cells[1].text = 'Script Mappings'
        hdr_cells[2].text = 'UAT Status'

        for status, data in table_data.items():
            os_tickets_str = ', '.join(', '.join(map(str, tickets)) for tickets in zip(data['os_tickets'][::2], data['os_tickets'][1::2]))
            script_mappings_str = ', '.join(', '.join(map(str, scripts)) for scripts in zip(data['script_mappings'][::2], data['script_mappings'][1::2]))
            row_cells = table.add_row().cells
            row_cells[0].text = os_tickets_str
            row_cells[1].text = script_mappings_str
